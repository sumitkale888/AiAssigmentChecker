...
const StudentClassCard: React.FC<StudentClassCardProps> = ({
  ...
  // Move random color inside component to get different colors per card
  ...
  return (
    <div className="w-72 bg-white border rounded-3xl border-gray-300 shadow-md overflow-hidden font-inter cursor-pointer" onClick={handleCardClick}>
      <div
        className={`relative w-full rounded-t-3xl ${randomColorClass} bg-cover bg-center flex flex-col justify-between p-4 text-white`}
        style={{
          height: '100px',
          backgroundImage: uploaded_photo_url ? `url(${uploaded_photo_url})` : 'none',
        }}
      >
